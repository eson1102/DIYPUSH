...
🤖 MindVideo 自动签到系统"""
    ...
    def send_notification(self):
        ...
        if self.checkin_result["success"]:
            title = f"MindVideo 签到成功 🎉 [{self.account_index}]"
...
{credit_info}"""
            ...
            title = f"MindVideo 签到失败 ⚠️ [{self.account_index}]"
            content = f"""
...
if __name__ == "__main__":
    ...
    # 格式示例：email1&password1,email2&password2
    ...
    print(f"\n{'=' * 60}")
    print(f"📊 全部完成！成功: {success_count}, 失败: {fail_count}, 总计: {len(accounts)}")
    print(f"{'=' * 60}")
